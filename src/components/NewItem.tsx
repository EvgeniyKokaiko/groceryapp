...
interface IProps {
    AddNewItem(photo: string, name:string, description: string, quantity: number, color: string, weight: string, width: number, height: number, depth: number, id: number): Function
    DataReducer: Item[]
}
...
const New = (props: IProps): JSX.Element => {
    ...
    const AddPostHandler = () => {
        if (name === "" || photo === "" || description === "" || quantity < 0 || color === "" || weight === "")  {
            setGood(0)
            setAdded(0)
        } else if (isGood === 1 && isAdded === 0) {
            props.AddNewItem(photo, name, description, quantity, color, weight, width, height, depth, props.DataReducer.length)
            setAdded(1)
        } else {
            setGood(1)
        }
                    ...
                    >
                        <>
                            ...
                            <div className="content">
                                <form className="ui form">
                                    ...
                                    <div className="field">
                                        ...
                                        <input type="text" value={photo} onChange={(e: ChangeEvent<HTMLInputElement>) => setPhoto(e.target.value)}/>
                                    </div>
                                    <br />
                                    <div className="field">
                                ...
                                {isGood === 0 && isAdded === 0 ? (
                                    <div className="ui negative message">
                                        ...
                                        <p>Fix it or refresh page</p></div>
                                ) :isGood === 1 && isAdded === 1?
                            ...
                            <div className="actions">
                                ...
                                <Link to="/" className="negative ui button">Cancel</Link>
                            </div>
                        </>
                    </div>
                </div>
...
export default connect(mapStateToProps, {AddNewItem})(New);
