...
export const AddNewItem = (photo: string, name:string, description: string, quantity: number, color: string, weight: string, width: number, height: number, depth: number ,id: number) => async (dispatch: Dispatch<DispatchObj>) => {
...
export const DeleteItem = (id: number) => async (dispatch: Dispatch<DispatchObj>) => {

    /***
     *   This method realized to delete item, and clear every comment which relate with this item
     */

        const response = await axios.get(`http://localhost:8888/comments?productId=${id}`)
...
export const FetchComments = (id: string) => async (dispatch: Dispatch<DispatchObj>) => {
    const response = await axios.get(`http://localhost:8888/comments?productId=${id}`)

    dispatch({type: ActionTypes.FetchComments, payload: response.data})
}

export const AddComment = (name: string, text: string, date: string, productId: string) => async (dispatch: Dispatch<DispatchObj>) => {
...
export const DeleteComment = (id: number) => async (dispatch: Dispatch<DispatchObj>) => {
